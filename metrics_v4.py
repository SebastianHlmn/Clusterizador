...
def _system_clause(path_str: str, only_accusatory: bool) -> str:
    """Filtra por sistema procesal cuando la fuente expone esa variable.

    Si la fuente ya es una extracción exclusivamente acusatoria y no conserva la
    variable de sistema, no agrega una condición redundante.
    """
...
def _excluded_org_clause(axis: str) -> str:
    """Exclusiones estructurales que no participan del universo comparable.

    Se aplican siempre, aun cuando se desactive el filtro de 'solo unidades
    fiscales', porque Justicia Nacional, Procuradurías y Casación no son unidades
    comparables con las unidades fiscales territoriales del análisis.
    """
