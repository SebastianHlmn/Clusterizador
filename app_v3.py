...
def set_list_callback(key: str, values: Sequence[str]) -> None:
...
def sync_multiselect(key: str, options: Sequence[str], preferred: Optional[Sequence[str]] = None) -> None:
    opts = list(options)
    if preferred is not None:
        current = list(preferred)
        valid = [value for value in current if value in opts]
...
def selection_row(label: str, key: str, options: Sequence[str], help_text: Optional[str] = None) -> list[str]:
    a, b, c = st.columns([6, 1, 1])
    with a:
        selected = st.multiselect(label, options=list(options), key=key, help=help_text)
    with b:
        st.write("")
        st.write("")
        st.button(
            ...
            on_click=set_list_callback,
            args=(key, list(options)),
        )
    with c:
        st.write("")
        st.write("")
...
def main() -> None:
    ...
    only_accusatory = st.checkbox(
        "Solo unidades del sistema acusatorio",
        value=True,
        key="cfg_only_accusatory",
        help=(
            "Filtra antes de calcular los indicadores. Se consideran acusatorias las filas cuya unidad fiscal "
            "comienza con 'Fiscalía' (con o sin tilde), según el eje Actuación / Actual / Ingreso elegido."
        ),
    )
    st.caption(
        "La selección es jerárquica. El filtro acusatorio se aplica antes de agregar casos, actuaciones, "
        "audiencias, hitos y conflictividad."
    )

    try:
        hierarchy = metrics.hierarchy_options(
            str(current_path), axis, start.isoformat(), end.isoformat(), only_accusatory
        )
    except Exception as exc:
        st.exception(exc)
        st.stop()
    ...
    if pending is not None:
        if "distritos" in pending:
            preferred_districts = pending.get("distritos", [])
        elif grain == "distrito":
            preferred_districts = pending.get("unidades", [])
    sync_multiselect("cfg_districts", district_options, preferred_districts)
    selected_districts = selection_row("Distrito", "cfg_districts", district_options)
    ...
    if grain in {"unidad", "oficina"}:
        unit_options = sorted(hierarchy_d["unidad_fiscal"].dropna().astype(str).unique().tolist())
        preferred_units = None
        if pending is not None:
            if "unidades_fiscales" in pending:
                preferred_units = pending.get("unidades_fiscales", [])
            elif grain == "unidad":
                preferred_units = pending.get("unidades", [])
        sync_multiselect("cfg_fiscal_units", unit_options, preferred_units)
        selected_fiscal_units = selection_row(
            "Unidad / sede fiscal", "cfg_fiscal_units", unit_options
        )
    ...
    if grain == "oficina":
        hierarchy_u = hierarchy_d[hierarchy_d["unidad_fiscal"].isin(selected_fiscal_units)].copy()
        office_options = sorted(hierarchy_u["oficina_etiqueta"].dropna().astype(str).unique().tolist())
        preferred_offices = None
        if pending is not None:
            if "oficinas" in pending:
                preferred_offices = pending.get("oficinas", [])
            else:
                preferred_offices = pending.get("unidades", [])
        sync_multiselect("cfg_offices", office_options, preferred_offices)
        selected_offices = selection_row(
            "Oficina / área",
            "cfg_offices",
            office_options,
            help_text="Se muestra como 'Unidad / sede · Oficina' para no confundir oficinas con igual nombre.",
        )
    ...
    with st.expander("Ver selección completa", expanded=False):
        st.write(analysis_units)
    ...
    with st.spinner("Construyendo matriz de unidades..."):
        try:
            ...
            core_df = metrics.build_core_metrics(
                ...
                analysis_units_tuple, only_accusatory
            )
    ...
    default_features = [feature for feature in dict.fromkeys(default_features) if feature in matrix.columns]
    default_features = [
    ...
    if pending is not None:
        loaded_features = [feature for feature in pending.get("indicadores", []) if feature in labels_map]
        selected_for_load = loaded_features or default_features
        st.session_state["cfg_features_labels"] = [labels_map[feature] for feature in selected_for_load]
...
- **Filtro acusatorio:** se aplica a las filas antes de cualquier agregación, según la unidad fiscal del eje elegido.
            """
        )
    ...
    with b:
        log_counts = st.checkbox(
            "Logaritmo en cantidades brutas", value=True,
            help="Conserva el tamaño pero reduce el dominio de unidades extremadamente grandes.",
            key="cfg_log_counts",
        )
    with ccol:
        algo = st.selectbox(
            "Algoritmo", ["Emergente automático", "Jerárquico Ward", "K-means", "DBSCAN"],
            index=0, key="cfg_algo"
        )
    with d:
    ...
    fig = px.scatter(
        work, x="PC1", y="PC2", color="cluster_etiqueta", hover_name="unidad",
        hover_data={
            "casos": ":,.0f", "actuaciones": ":,.0f", "audiencias": ":,.0f",
            "PC1": ":.2f", "PC2": ":.2f"
        },
    ...
    if not scores.empty:
        with st.expander("Elección del número de clusters", expanded=False):
            fig_k = px.line(
                scores, x="k", y="silhouette", markers=True,
    ...
    prof = prof.reset_index().rename(columns={column: labels_map.get(column, column) for column in prof.columns})
    ...
    with st.expander("Tabla completa", expanded=False):
        show_cols = ["unidad", "cluster_etiqueta"] + [feature for feature in features if feature in work.columns]
        st.dataframe(
            work[show_cols].rename(columns={column: labels_map.get(column, column) for column in show_cols}),
    ...
    st.download_button(
        "Descargar matriz y clusters (CSV)", data=csv,
        file_name=f"clusters_unidades_{grain}_{start}_{end}.csv", mime="text/csv"
    )
